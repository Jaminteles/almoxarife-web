...
export default function ServicosList() {
  ...
  const [filtros, setFiltros] = useState({ data: "", fornecedor: "", produto: "", aplicacao: "" });
  ...
  const carregar = (ativos = {}) => {
    ...
    fetch(`${API_URL}/servicos${params.toString() ? `?${params}` : ""}`).then((r) => r.json()).then((res) => {
      ...
      setRegistros(res.dados.map((s) => ({ Data: data(s.data_servico), Fornecedor: s.fornecedor?.razao_social || s.fornecedor?.nome_fantasia || "—", Responsável: s.responsavel?.nome || "—", Aplicação: s.aplicacao, Produtos: s.itens?.map((i) => i.produto?.nome || `Produto ${i.id_produto}`).join(", ") || "—", Total: moeda(s.valor_total), __id__: s.id_servico })));
      setLoading(false);
  ...
  return <Container maxWidth={false} disableGutters>
    ...
    <ListTemplate modulo="servicos" title="Serviços" columns={["Data", "Fornecedor", "Responsável", "Aplicação", "Produtos", "Total"]} data={registros} loading={loading} onCreate={() => navigate("/servicos/cadastro")} onEdit={(item) => navigate(`/servicos/${item.__id__}/editar`)} onInactivate={setConfirmar} onSearch={() => carregar(filtros)} onClear={() => { const limpos = { data: "", fornecedor: "", produto: "", aplicacao: "" }; setFiltros(limpos); carregar(); }} emptyMessage="Nenhum serviço encontrado." actionLabel="Excluir" actionIcon={<DeleteOutlineIcon fontSize="small" />} actionColor="error.main" filters={<>
      ...
      <TextField select label="Fornecedor" size="small" value={filtros.fornecedor} onChange={(e) => setFiltros({ ...filtros, fornecedor: e.target.value })} sx={{ minWidth: 180 }}><MenuItem value="">Todos</MenuItem>{fornecedores.map((f) => <MenuItem key={f.id_fornecedor} value={f.id_fornecedor}>{f.razao_social || f.nome_fantasia}</MenuItem>)}</TextField>
